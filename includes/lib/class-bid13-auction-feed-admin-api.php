...
class Bid13_Auction_Feed_Admin_API {
	...
	public function display_field ( $data = array(), $post = false, $echo = true ) {
		...
		switch( $field['type'] ) {
			...
			case 'image':
				...
				$html .= '<input id="' . $option_name . '_button" type="button" data-uploader_title="' . __( 'Upload an image' , 'bid13-auction-feed' ) . '" data-uploader_button_text="' . __( 'Use image' , 'bid13-auction-feed' ) . '" class="image_upload_button button" value="'. __( 'Upload new image' , 'bid13-auction-feed' ) . '" />' . "\n";
				$html .= '<input id="' . $option_name . '_delete" type="button" class="image_delete_button button" value="'. __( 'Remove image' , 'bid13-auction-feed' ) . '" />' . "\n";
				$html .= '<input id="' . $option_name . '" class="image_data_field" type="hidden" name="' . $option_name . '" value="' . $data . '"/><br/>' . "\n";
			break;

			case 'color':
				?><div class="color-picker" style="position:relative;">
			        <input type="text" name="<?php echo sanitize_text_field( $option_name ); ?>" class="color" value="<?php echo sanitize_text_field( $data ); ?>" />
	...
	public function validate_field ( $data = '', $type = 'text' ) {
		...
		switch( $type ) {
			...
			case 'url': $data = esc_url_raw( $data ); break;
